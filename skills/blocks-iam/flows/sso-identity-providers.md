...
(`/auth/identity-providers*`, `/auth/social/*`, `/auth/login-options`).

1. `POST /auth/identity-providers` — register the provider. Schema-required
...
2. Manage: `GET /auth/identity-providers` (list), `GET /{id}` (single — secrets
   are never returned), `PUT /{id}` (update; send the full config again),
   `PATCH /auth/identity-providers/{id}/status` with `{ "isActive": false }` to
   disable without deleting (preferred over deletion; no DELETE route exists in v4).

3. `GET /auth/login-options` — public, unauthenticated. Confirm the provider now
...
4. Runtime login round trip (your UI, API pattern):
   1. `GET /auth/social/initiate?redirectUri=<your callback>` — the
   ...
   2. Provider authenticates the user and redirects back to your app with
      `code` + `state`.
   3. `POST /auth/social/callback` with snake_case body
   ...
   Alternative hosted pattern: SPA `fetch`es `GET /idp/initiate?clientId=&redirectUri=`,
   the platform returns a redirect URL in the JSON body (typically `redirect_uri`
   per the live `iam/v4` response), the SPA navigates there with `window.location.href`,
   and the platform handles the return at `GET /idp/callback` (endpoints.md →
   [Idp](../endpoints.md#idp)); UI hints via `GET /idp/oidc-ui-config`. For the full
...
5. SSO signups: allow them via `POST /iam/signup-settings`
...
1. `POST /oidc-clients` — upsert your app's client registration: `redirectUris`,
   ...
   `GET /oidc-clients`, `GET|DELETE /oidc-clients/{clientId}` (delete revokes
...
3. Code flow: send the browser to `GET /oidc/authorize` with the standard query
   ...
   `POST /oidc/token` (supports authorization_code and refresh_token grants; the
   ...
   flows, `POST /oidc/login` authenticates credentials and issues the
   authorization code in one call (snake_case body incl. `code_challenge`, plus
   `mfa_id`/`mfa_code`/`captcha_code` branches).

4. Session management (hosted login, multi-account): `GET /oidc/session`,
   `GET /oidc/session/accounts`, `POST /oidc/session/account/add`,
   `POST /oidc/session/account/select`, `DELETE /oidc/session/accounts/{userId}`,
   and `POST /oidc/session/revoke` (logout of all accounts in the session).
   These return typed shapes and RFC 7807 `ProblemDetails` on 4xx — see
   endpoints.md → [IdpSession](../endpoints.md#idpsession).

## Verify

- Part A: `GET /auth/login-options` lists the provider; complete one social
  login round trip and confirm `GET /auth/me` returns claims; the new user shows
  `externalIdentities` (admin: `GET /iam/users/{id}`) and got `initialRoles`.
- Part B: `.well-known/openid-configuration` resolves for your `tenant_id`;
  authorize → token round trip yields tokens that pass
  `POST /oidc/introspect` (see [machine-to-machine.md](machine-to-machine.md)).
