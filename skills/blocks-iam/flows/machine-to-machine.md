...
1. `POST /api/auth/client-credentials` — create the credential:
...
2. `GET /api/auth/client-credentials` — list credentials; each item includes
...
3. Token exchange for the service: **not documented in the v4 swagger.**
   `POST /api/oidc/token` is the platform's OAuth 2.0 token endpoint, but its
...
4. Rotate by creating a replacement credential, migrating the consumer, then
   `POST /api/auth/client-credentials/delete` with `{ "itemId": "<old credential>" }`
...
- `POST /api/oidc/introspect` — RFC 7662 token introspection: authorized clients ask
...
- `POST /api/oidc/revoke` — RFC 7009 revocation of access/refresh tokens (e.g. on
  credential compromise).
- `GET /api/oidc/revocation-history` — audit trail of revocations.

## Verify

- `GET /api/auth/client-credentials` shows the credential with `isActive: true` and
  the expected `roles`/`permissionsByOrg`.
- After deletion, the credential no longer appears in the list.
- A token you revoked via `POST /api/oidc/revoke` subsequently introspects as
  inactive via `POST /api/oidc/introspect` (exact response field undocumented —
