...
1. *(Optional)* `GET /auth/login-options` — public discovery endpoint; returns the
...
2. `POST /auth/login` — the credential exchange
   ...
   **Branch — MFA challenge.** If the account has MFA enabled, the first login call
   ...
   `POST /auth/login` again with the same credentials plus:
   ...
   `POST /mfa/backup-codes/use` with `{ "userId", "code" }` (response undocumented).

   **Branch — account locked.** Repeated failures lock the account for
   `accountLockDurationInMinutes` (tenant setting, `GET/POST /auth/config`).

3. `GET /auth/me` — with `Authorization: Bearer <access_token>`. Returns OIDC
...
4. *(Optional)* `GET /iam/me` — the IAM profile document, enveloped as
   `{ data, errors }` where `data` is an open key/value map. Use this (and
   `PATCH /iam/me` for self-service profile edits: `firstName`, `lastName`,
   `phoneNumber`, `profileImageUrl`, …) when you need profile data rather than
   token claims.

5. Keep the session alive: `POST /auth/refresh` before the access token expires:
   ...
   readable/settable via `GET/POST /auth/config` (admin). On a 401 anywhere in
   your app: refresh once, retry, and force re-login if the refresh also fails
   (pattern detailed in `blocks-setup`).

6. Log out:
   - Current session: `POST /auth/logout` with `{ "refreshToken": "<refresh_token>" }`.
     **Note the camelCase `refreshToken` here** — unlike refresh's `refresh_token`.
   - All devices: `POST /auth/logout-all` with `{ "useBackchannel": true }`
...
1. `POST /mfa/totp/setup` — starts enrollment (expect a secret / otpauth URI for
   the QR code — verify live).
2. `POST /mfa/totp/verify-setup` with `{ "code": "123456" }` — confirms the
   authenticator app is synced.
3. `POST /mfa/backup-codes/generate` then `GET /mfa/backup-codes` — issue and
   show recovery codes once.
4. `GET /mfa/status` — confirm enrollment; `PUT /mfa/preferred-method` with
   `{ "mfaType": <0-4> }` to pick the default method.

Tenant-wide enforcement (require MFA for roles, allow opt-out, backup-code count)
is `PUT /mfa/policy`. An admin can clear a locked-out user's MFA with
`POST /mfa/admin/reset` `{ "userId", "reason" }`.

## Verify

- `GET /auth/me` returns 200 with the expected user claims.
- `POST /auth/refresh` returns a new token pair, and the new access token works
  against `GET /auth/me`.
- After logout, the revoked refresh token is rejected by `POST /auth/refresh`.
- Admin view: the login appears in `GET /iam/sessions?Filter.UserId=<id>` and
  `GET /iam/history?Filter.UserId=<id>`.
