...
### 2. (Optional) Check the code — `POST /iam/v4/auth/validate-activation`

Checks validity **without** activating — useful before prompting the user for a password.

```bash
curl -s -X POST "$BLOCKS_API_URL/iam/v4/auth/validate-activation" \
...
### 3. Activate — `POST /iam/v4/auth/activate`

Consumes the code, sets the password, and marks the account active.

```bash
curl -s -X POST "$BLOCKS_API_URL/iam/v4/auth/activate" \
  -H "x-blocks-key: $X_BLOCKS_KEY" \
  -H "Content-Type: application/json" \
...
### 4. Branch — resend — `POST /iam/v4/auth/resend-activation`

Generates a new activation code and emails it to the user.

```bash
curl -s -X POST "$BLOCKS_API_URL/iam/v4/auth/resend-activation" \
  -H "x-blocks-key: $X_BLOCKS_KEY" \
  -H "Content-Type: application/json" \
...
(`POST /iam/users`). Then repeat steps 2–3 with the code from the **newest** email.
...
`POST /auth/recover` + `POST /auth/reset-password` — documented in `blocks-iam`.

## Verify

- `POST /iam/v4/auth/login` with the account's credentials returns tokens instead of 401
  (response shape not documented in swagger — inspect the live payload).
- `GET /iam/v4/auth/me` with the new access token returns 200 with the user's claims — this is
