...
1. `POST /api/Workflow/Create` — create the workflow shell.
...
2. `POST /api/Workflow/GetAll` — find the workflow and capture its id.
...
3. `PUT /api/Workflow/Update` — set the graph.
...
4. Publish so it can run — `POST /api/Workflow/PublishNewVersion`.
...
5. Trigger a run — `POST /api/Workflow/Webhook/{projectKey}/{workflowId}/{webhookId}`.
   ...
   it from the workflow definition (`GET /api/Workflow/Get`). Send the trigger payload as
   ...
   `POST /api/Workflow/webhook-test/{projectKey}/{workflowId}/{webhookId}` (note the
   lowercase route — casing is exact).

6. Debug a single node — `POST /api/Workflow/StepExecute`.
...
- Webhook returns an error / no execution appears → confirm the workflow is published
  (`POST /api/Workflow/GetAll` with `isPublished: true` should include it) and that
...
- `POST /api/Workflow/GetAll` with `isPublished: true` lists the workflow.
- After the webhook call, `GET /api/Workflow/GetExecutions?ProjectKey=...&WorkflowId=...`
  shows a new execution; `GET /api/Workflow/GetExecution` for that id shows the run's
