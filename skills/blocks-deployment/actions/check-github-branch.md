# Action: check-github-branch

## Purpose

Check if a specific branch exists in a GitHub repository.

---

## Endpoint

```
GET $API_BASE_URL/cloudbuild/v1/Github/GithubBranchExists?repoId={id}&ProjectKey=$PROJECT_SLUG
```

---

## curl

```bash
curl --location "$API_BASE_URL/cloudbuild/v1/Github/GithubBranchExists?repoId=repo-id-here&ProjectKey=$PROJECT_SLUG" \
  --header "x-blocks-key: $PROJECT_SLUG" \
  --header "Content-Type: application/json"
```

---

## Query Parameters

| Param | Type | Required | Notes |
|-------|------|----------|-------|
| repoId | string | yes | Repository ID |
| ProjectKey | string | yes | Use `$PROJECT_SLUG` (PascalCase) |

---

## On Success (200)

```json
{
  "isSuccess": true,
  "errors": null,
  "message": "string",
  "statusCode": 200,
  "data": {
    "exists": true,
    "branchName": "main"
  }
}
```

---

## On Failure

| HTTP Status | Cause | Action |
|-------------|-------|--------|
| 401 | Missing or invalid `x-blocks-key` | Verify project key |
| 404 | Repository not found | Verify `repoId` |
