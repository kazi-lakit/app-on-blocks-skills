...
Replace the ngx-translate HTTP loader to use `getUilmFile`, with local file fallback when the UILM API is unavailable:
...
export class BlocksTranslateLoader implements TranslateLoader {
  constructor(
    ...
    private localFallbackDir: string = 'src/assets/i18n'
  ) {}

  private readLocalFile(lang: string, namespace: string): Record<string, string> {
    const filePath = path.resolve(this.localFallbackDir, `${lang}.json`);
    try {
  ...
  getTranslation(lang: string): Observable<Record<string, string>> {
    ...
    const requests = this.namespaces.map(ns => {
      ...
      return this.http.get<Record<string, string>>(
        ...
        catchError(() => {
          const localFallback = this.readLocalFile(lang, ns);
...
> [!IMPORTANT]
> The error handler uses `fs.readFileSync` to read local translation files as a real fallback — it does **not** return `{}`. If UILM is unreachable, the loader reads from `src/assets/i18n/{lang}.json` instead. Keep local translation files in that directory for offline operation.
