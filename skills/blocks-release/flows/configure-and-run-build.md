...
1. `GET /Build/repos-list` — list repos the release service knows about; pick the target and
   ...
   dedicated "connect repo" endpoint is exposed in the v4 swagger; `POST /Build/repo-update`
   only associates repo↔domain pairs, see `custom-domains.md`).

2. `GET /Build/settings` — fetch the hosting catalog available to this project. Response shape
...
3. `GET /Build/repo-details?RepoId=<repoId>` — current settings for the repo (note PascalCase
   `RepoId`). Response shape not documented in swagger. If the repo already has the desired
   hosting configuration, skip step 4.

4. `POST /Build/repo-settings-update` — persist hosting/deployment settings. Body
...
5. `POST /Build/manual` — trigger the build. Body (`RepoBuildRequest`):
   ...
   Alternative: `POST /Build/run-build` takes the same body, but its response and its
...
6. Poll `GET /Build?buildId=<buildId>` until the build reaches a terminal state. Response
...
7. `GET /Build/reports?buildId=<buildId>` — pull reports/artifacts once terminal (see
...
- `GET /Build?buildId=…` shows a terminal success state.
- `GET /Build/repo-details?RepoId=…` reflects the settings you wrote in step 4 (field names in
