...
(`POST /Build/manual` returns one — see `configure-and-run-build.md`).
...
1. Get a `buildId`. Either from the trigger response (`BuildResponse.buildId`) or by asking the
   user / checking OS portal build history. `GET /Build?buildId=<id>` confirms the build
   exists and its state (response shape not documented in swagger — inspect).

2. `GET /Build/reports?buildId=<buildId>` — fetch all reports for the build. Response shape
   not documented in swagger — inspect the live response before parsing.

3. Optionally filter: `GET /Build/reports?buildId=<buildId>&type=<type>`. The set of valid
...
4. Wire SonarQube: `GET /AnalyticsTool/ProcessSonarQubeUser?buildId=<buildId>`.
...
5. Wire DependencyTrack: `GET /AnalyticsTool/ProcessDependencyTrackUser?buildId=<buildId>`.
...
- **Empty/404-ish reports on a fresh build** → reports may only exist after the build reaches a
  terminal state; poll `GET /Build?buildId=…` first.
