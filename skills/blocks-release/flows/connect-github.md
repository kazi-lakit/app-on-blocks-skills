...
1. `GET /Auth/TestPing` — optional connectivity sanity check before debugging anything else.
   Response shape not documented in swagger — treat any 200 as "service reachable".

2. `GET /Auth/IsAuthorized` — is a GitHub account already linked to this project?
...
3. Authorize GitHub. The OAuth *initiation* (GitHub consent screen) is **not exposed in the v4
   ...
   - `GET /Auth/AccessToken?code=<oauth_code>` — exchanges a GitHub OAuth `code` for a stored
...
4. `GET /Github/user` — confirm *which* GitHub account is linked (avoid building against the
   wrong org/user). Response shape not documented in swagger — inspect; expect GitHub account info.

5. `GET /Github/repos?Search=<filter>&PageNumber=1&PageSize=20` — find the repository.
...
6. `GET /Github/branches?repo=<owner/name>` — list branches so the user can pick a deploy
   branch. For a quick existence check on an already-registered repo there is also
   `GET /Github/GithubBranchExists?repoId=<repoId>`. Note the casing: `repo` here, `repoId`
   there. Response shapes not documented in swagger.

7. `GET /Github/CreateWebhook?RepoId=<repoId>` — register the push webhook on the repo.
...
8. Nothing to call for the webhook itself: GitHub will now POST to
   `POST /Github/webhook?x-blocks-key=<key>` on every push (that is why the key is a query
...
- **Disconnect GitHub** → two endpoints exist and the swagger does not document the difference:
  `DELETE /Auth/DeleteAuthorization` and `POST /Auth/RemoveAuthorization`. Try
  `DELETE /Auth/DeleteAuthorization` first; verify with `GET /Auth/IsAuthorized`. If the
  authorization persists, try `POST /Auth/RemoveAuthorization`. Report which one worked so the
  user can note it.
- **Repo missing from `/Github/repos`** → the linked account lacks access to it, or the OAuth
  grant didn't include that org. Re-authorize with the right account/org scope (step 3).

## Verify

- `GET /Auth/IsAuthorized` reflects the linked state.
- `GET /Github/user` returns the expected account.
- On GitHub: repo → Settings → Webhooks shows a hook pointing at
  `…/release/v4/Github/webhook` after step 7.
- End-to-end: push a trivial commit to the deploy branch, then check that a new build appears
  (`GET /Build?buildId=…` once you have an id, or via the configure-and-run-build flow's
