...
| I need to… | Go to |
|---|---|
| Check whether GitHub is authorized | `GET /api/Auth/IsAuthorized` — endpoints.md#auth |
| Authorize / disconnect GitHub | flows/connect-github.md |
| See the linked GitHub account | `GET /api/Github/user` — endpoints.md#github |
...
| See hosting options (providers, regions, machine specs) | `GET /api/Build/settings` — endpoints.md#build |
| List repos known to the release service | `GET /api/Build/repos-list` — endpoints.md#build |
...
| Check one build's status | `GET /api/Build?buildId=…` — endpoints.md#build |
...
| Log a user into my app (tokens, MFA) | **blocks-iam** skill — the `/api/Auth/*` controller *here* is GitHub authorization, not app login |
...
- **Repo** — a GitHub repository visible to the service. GitHub-side identity comes from `/api/Github/repos`; once registered for builds it appears in `/api/Build/repos-list` and is addressed by `repoId`.
- **Build** — one CI run for a repo, addressed by `buildId` (returned by `POST /api/Build/manual`). Status via `GET /api/Build`, artifacts via `GET /api/Build/reports`.
...
- **Webhook** — a GitHub push hook registered by `GET /api/Github/CreateWebhook`; GitHub then calls `POST /api/Github/webhook` on every push, which triggers auto-deploy.
...
- **Sparse response documentation.** Only two endpoints in this service document a response schema: `POST /api/Build/manual` (`BuildResponse`, carries `buildId`) and `POST /api/Build/repo-update` (`BaseApiResponse`). Every other endpoint returns "200 OK, no schema" in swagger — inspect the live response before writing code that depends on its shape, and say so in generated code comments.
...
- **Mutating GETs.** `GET /api/Github/CreateWebhook` and `GET /api/Github/clone` perform actions despite being GETs. Don't call them speculatively or in prefetch/retry loops.
