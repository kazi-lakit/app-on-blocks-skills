...
- Most endpoints take a `projectKey` — **projectKey = your Blocks Key** (the same value as `X_BLOCKS_KEY`). It is distinct from `projectShortKey` (the short slug used in the GraphQL gateway path), which is runtime-discovered, not an env var — read it from `GET /configurations` (`data.projectShortKey`).
- A data source must be configured before schemas behave fully — check with `GET /configurations` (see [endpoints.md#configuration](endpoints.md#configuration)).
...
| Check or change the database connection | [endpoints.md#configuration](endpoints.md#configuration) (`GET/POST/PUT /configurations`) |
...
- **Data source** — the MongoDB connection backing the project (`connectionString`, `databaseName`, plus `collectionNamePattern` / `isCollectionNameEditable` returned by `GET /configurations`).
...
- **Files / DMS** — document management embedded in the data service: pre-signed upload URLs, folders, tags, metadata, versions. Routes are PascalCase (`/Files/*`).
...
- **Reload or it didn't happen:** schema/field/policy/validation changes are staged until you call `POST /schema-configurations/reload`. Check pending changes with `GET /schemas/unadapted-change-logs`.
- **Pagination:** list endpoints (`GET /schemas`, `GET /data-validations`) use **PascalCase query params** — `PageNo`, `PageSize`, `SortBy`, `SortDescending`, `Keyword`, `ProjectKey`. Files listing (`POST /Files/GetFilesInfo`) instead takes a camelCase JSON body (`page`, `pageSize`, `sort`, `filter`).
- **Casing quirks:** `/Files/*` routes and `GET /Files/GetFile`'s query params (`FileId`, `Version`, `ConfigurationName`, `ProjectKey`) are PascalCase; everything else is lowercase kebab with camelCase bodies.
...
- **Mock-data routes:** read with `GET /mock-data` (swagger documents the path as `GET /mock-data/mock-data` — verify which responds in your project) and delete with `DELETE /mock-data`.
- **Undocumented responses:** all `/data-access/policy/*` endpoints, `POST /regex/generateregex`, and `POST /Files/updateFileAdditionalInfo` have **no response schema in swagger** — inspect the live response before relying on a shape.
...
- **GraphQL runtime gateway is not in the swagger.** The v1 pattern was `POST /uds/v1/{projectShortKey}/gateway`; the v4 equivalent is likely `POST https://api.seliseblocks.com/data/v4/{projectShortKey}/gateway` but this is **unverified — verify against your project** before wiring it in. (`projectShortKey` is runtime-discovered — read it from `GET /configurations` → `data.projectShortKey`; it is not an env var.) All REST configuration flows in this skill are verified; keep them primary. `SchemaDefinitionResponse.querySchema` / `mutationSchemas` hint at the generated GraphQL operation names per schema.
