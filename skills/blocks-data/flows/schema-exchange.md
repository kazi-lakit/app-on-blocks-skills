...
3. Wait for the export notification (or poll the file): the exported JSON lands in blob storage as a file. You can confirm it exists with `GET /api/Files/GetFile?FileId=<fileId>&ProjectKey=<source Blocks Key>` (see [upload-files.md](upload-files.md)) — the `url` lets you inspect the JSON before importing.
...
- `GET /api/schemas?ProjectKey=<target Blocks Key>&PageSize=100` — the exported schemas appear in the target with their fields.
- Spot-check one schema: `GET /api/schemas/info-by-name?schemaName=<name>&projectKey=<target Blocks Key>`.
- `GET /api/schemas/unadapted-change-logs?projectKey=<target Blocks Key>` — empty after the final reload.
