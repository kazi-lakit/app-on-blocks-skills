...
- **`GET /api/Mfa/SetUpTotp?ProjectKey=<key>&UserId=<id>`** — returns `{ qrImageUrl, qrCode, isSuccess, errors }`. Show the QR (image URL or raw code for manual entry) to the user; they scan it into their authenticator app. Verify the first generated code via `POST /api/Mfa/VerifyOTP` (step 5) to complete enrollment.

### Reset / disable branch

- **`POST /api/Mfa/DisableUserMfa`** — `{ userId, projectKey }` clears MFA for one user (e.g. lost device). Returns `BaseResponse`.
- To turn MFA off project-wide, `POST /api/Mfa/Save` with `enableMfa: false`.

### Enforcement branch

- To require MFA on specific platform endpoints: find configs via `POST /api/ApiEndpointConfig/GetList`, then set `isMfaRequired: true` via `POST /api/ApiEndpointConfig/Update` or `/BulkUpdate` ([endpoints.md#apiendpointconfig](../endpoints.md#apiendpointconfig)). Responses undocumented in swagger — verify live.
...
- `GET /api/Mfa/Get` reflects the saved configuration (`enableMfa: true`, expected `userMfaType`).
