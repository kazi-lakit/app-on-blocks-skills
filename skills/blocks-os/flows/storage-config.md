...
1. **`POST /Storage/Save`** — register the backend ([endpoints.md#storage](../endpoints.md#storage)). Common fields: `name`, `storageStrategy` (string naming the strategy — values not enumerated in swagger; check OS portal), `projectKey`, `updateRequest: false`. Then per backend type:
...
2. **`GET /Storage/Get?ProjectKey=<key>&ConfigurationName=<name>`** — read one configuration back. The response includes the stored credential fields (`connectionString`, `secretKey`, `accessKey`, `password`, `sftpSecretKey`) — handle and log it accordingly.
3. **`GET /Storage/Gets?ProjectKey=<key>`** — list every configuration for the project. Response is a bare array of `StorageConfiguration` (no envelope).
4. **Rotate credentials: `POST /Storage/Save` with `updateRequest: true` and `itemId`** — resend the configuration with new keys. There is no separate Update endpoint; `updateRequest` is the switch (note: this controller calls it `updateRequest`, Notification calls it `isUpdateRequest` — copy per controller).
5. **`POST /Storage/Delete?ProjectKey=<key>&ConfigurationName=<name>`** — remove a configuration. Quirk: this is a **POST with query parameters and no body** — do not send JSON.
...
- `GET /Storage/Gets?ProjectKey=<key>` includes the configuration with the expected `name` and `storageStrategy`; it disappears after Delete.
- `GET /Storage/Get?ProjectKey=<key>&ConfigurationName=<name>` reflects rotated credentials after an `updateRequest: true` save.
