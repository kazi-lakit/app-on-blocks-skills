...
1. *(once per project, optional but recommended)* `POST /api/MagicLink/SaveConfig` — set up the link-based action config: `contextName`, `shortUrlBase` (e.g. `https://short.example.com/`), `projectKey` (projectKey = your Blocks Key, the same value as `X_BLOCKS_KEY`). Response returns `configId` and `wasCreated`. Keep `configId` if you want to pin links to it via `linkBasedActionConfigId`.
   - Check what exists first with `GET /api/MagicLink/GetConfig?ProjectKey=$X_BLOCKS_KEY` — `config` is null when nothing is configured yet.
...
- `GET /api/MagicLink/GetLink?ItemId=<linkId>&ProjectKey=$X_BLOCKS_KEY` — confirm `data.status` is `Active`, and after an invoke check `data.usageCount` incremented.
- `GET /api/MagicLink/GetLinks?ProjectKey=$X_BLOCKS_KEY&PageSize=10&PageNumber=0` — administrative listing; filter with `Type` (`Redirect`/`Action`), `Status` (`Active`, `TimeExpired`, `UsageLimitExceeded`, `ManuallyDisabled`), `SearchText` (matches Name and Uri), `RequestMethod`, or `ExpiryDateRange.StartDate`/`EndDate`.
