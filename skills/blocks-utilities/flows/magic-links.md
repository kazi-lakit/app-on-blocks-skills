...
1. *(once per project, optional but recommended)* `POST /MagicLink/SaveConfig` — set up the link-based action config: `contextName`, `shortUrlBase` (e.g. `https://short.example.com/`), `projectKey` (projectKey = your Blocks Key, the same value as `X_BLOCKS_KEY`). Response returns `configId` and `wasCreated`. Keep `configId` if you want to pin links to it via `linkBasedActionConfigId`.
   - Check what exists first with `GET /MagicLink/GetConfig?ProjectKey=$X_BLOCKS_KEY` — `config` is null when nothing is configured yet.

2. `POST /MagicLink/CreateLink` — create the link. Key fields:
   ...
   - `projectKey`: always pass your Blocks Key.
   - Keep from the response: `linkId` and `shortUri` (the shareable URL). Check `isSuccess`; on failure read `errorMessage`/`errors`.

3. Deliver the link. `shortUri` is what you hand to the recipient (email, QR code, SMS). The underlying invocation endpoint is `GET /MagicLink/Invoke/{linkId}` (optionally `?projectKey=...&subscriptionFilterId=...`).

4. Recipient opens the link → `GET /MagicLink/Invoke/{linkId}`:
...
Bulk variant: `POST /MagicLink/CreateLinks` with `requests: CreateMagicLinkRequest[]` and a default `projectKey`. Response gives per-link results (`links[].id`, `links[].shortUri`, `links[].isSuccess`, `links[].errorMessage`) plus `totalSuccessCount` — check each entry, a 200 with `isSuccess: true` at the top can still contain individual failures.

Revoke: `POST /MagicLink/RemoveLinks` with `linkIds: string[]` + `projectKey`. Response `removedCount` tells you how many were disabled. Removed links report status `ManuallyDisabled`.

## Verify

- `GET /MagicLink/GetLink?ItemId=<linkId>&ProjectKey=$X_BLOCKS_KEY` — confirm `data.status` is `Active`, and after an invoke check `data.usageCount` incremented.
- `GET /MagicLink/GetLinks?ProjectKey=$X_BLOCKS_KEY&PageSize=10&PageNumber=0` — administrative listing; filter with `Type` (`Redirect`/`Action`), `Status` (`Active`, `TimeExpired`, `UsageLimitExceeded`, `ManuallyDisabled`), `SearchText` (matches Name and Uri), `RequestMethod`, or `ExpiryDateRange.StartDate`/`EndDate`.
